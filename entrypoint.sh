...
STARTER_NAME="${3:-name}"
BRANCH_NAME="${4:-main}"
GITHUB_EMAIL=${5:-actions-github-push-subdirectories@design4.pro}
BASE=$(pwd)
...
for folder in $FOLDER/*; do
  ...
  git clone --depth 1 https://$GH_TOKEN@github.com/$GITHUB_USERNAME/$NAME.git $CLONE_DIR &> /dev/null
  cd $CLONE_DIR
  ...
  if [ -n "$(git status --porcelain)" ]; then
    ...
    git add .
    git commit --message "chore(package): update $NAME from $GITHUB_REPOSITORY"
